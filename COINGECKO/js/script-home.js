...
function mostra_tabela(){
    ...
    if(j == 0){
        ...
        $(clone[index]).val(index);
        $('#mensagem', clone[index]).text("Não existe nenhum item que corresponda à pesquisa!");
        $('#tbody').append(clone[index]);
    }
...
// Funçao que redireciona para a pagina detalhes
...
function search_coin() {
    ...
    for (i = 0; i < coins.length; i++) { 
        if (coins[i].nome.toLowerCase().includes(input) || coins[i].abreviatura.toLowerCase().includes(input) || coins[i].id  == input) {
